...
WITH ctePRE
     ...
     INTO #t
     FROM ctePRE b
          LEFT OUTER JOIN ctePOST e ON b.operation_id = e.operation_id
                                       AND b.package_name = e.package_name
                                       AND b.message_source_id = e.message_source_id
          INNER JOIN [catalog].executions e2 ON b.operation_id = e2.execution_id
...
WITH cte
     ...
     SELECT TOP 100 c.operation_id
                  ...
                  , [message]
     FROM cte AS c
          INNER JOIN #t t ON c.operation_id = t.operation_id
                             AND c.event_message_id BETWEEN t.from_event_message_id AND t.to_event_message_id
     WHERE subcomponent_name = 'SSIS.Pipeline'
           AND [message] LIKE '%rows.%'
     ORDER BY c.operation_id DESC
            , message_time DESC;